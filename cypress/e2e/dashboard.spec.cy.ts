...
describe('DashBoard', () => {
  ...
  it('Shall Post/Comment CRUD works fine', () => {
    cy.get('input[placeholder="New post ?"]').type('test post A')
    cy.get('[data-testid="btn-post"]').click()
    cy.get('[data-testid="ul-post"]').children().should('have.length', 1)
    cy.get('[data-testid="ul-post"]')
      .children()
      .should('have.text', 'test post A')
    cy.get('[data-testid="pencil-post"]').click()
    cy.get('input[placeholder="New post ?"]').type(' edited')
    cy.get('[data-testid="btn-post"]').click()
    cy.get('[data-testid="ul-post"]')
      .children()
      .should('have.text', 'test post A edited')
